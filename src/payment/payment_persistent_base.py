import logging
from firebase_admin import db
from src.common.utils import logger


class PaymentPersistentBase:
    """Base class added persistent methods"""
    dt_name = "Subscriptions"
    node_name = "payments"

    def create(self):
        """
        Creates a Player to the database
        """
        logger.info("Creating %s", self.id)
        # self.uid = None  # id must be none to generate next primary key
        subs_ref = db.reference(self.dt_name).child(str(self.pl_id)).child(str(self.sub_id)).child(self.node_name).child(str(self.id))
        subs_ref.set(self.serialize())
        logger.info("Created %s successfully", self.id)

    def update(self):
        """
        Updates a Player to the database
        """
        logger.info("Updating %s", self.id)
        subs_ref = db.reference(self.dt_name).child(str(self.pl_id)).child(str(self.sub_id)).child(self.node_name).child(str(self.id))
        subs_ref.update(self.serialize())

    def delete(self):
        """Removes a Player from the data store"""
        logger.info("Deleting %s", self.id)
        db.reference(self.dt_name).child(str(self.pl_id)).child(str(self.sub_id)).child(self.node_name).child(str(self.id)).delete()

    @classmethod
    def all(cls, pl_id, sub_id):
        """Returns all the records in the database"""
        logger.info("Processing all Player-transaction records")
        player_ref = db.reference(cls.dt_name).child(str(pl_id)).child(str(sub_id)).child(cls.node_name).get()
        data = []
        if player_ref is not None:
            for val in player_ref:
                if val is not None:
                    pays = cls.deserialize(val)
                    data.append(pays)
        return data

    @classmethod
    def all_json(cls, pl_id, sub_id):
        """Returns all the records in the database"""
        logger.info("Processing all Player-transaction records")
        player_ref = db.reference(cls.dt_name).child(str(pl_id)).child(str(sub_id)).child(cls.node_name).get()
        if player_ref is not None:
            for val in player_ref:
                if val is not None:
                    logger.info("Val in ref: ", val)
            return player_ref
        return []

    @classmethod
    def check_if_exist(cls, pl_id, uid):
        """check if record is exist in database"""
        logger.info("check if data exist")
        subs_ref = db.reference(cls.dt_name).child(str(pl_id)).child(str(uid)).child(cls.node_name).get()
        if subs_ref is not None:
            return True
        return False

    @classmethod
    def find(cls, player_id, by_uid, id):
        """Finds a record by its ID"""
        logger.info("Processing lookup for id %s ...", by_uid)
        subs_ref = db.reference(cls.dt_name).child(str(player_id)).child(str(by_uid)).child(cls.node_name).child(str(id))
        if subs_ref.get() is not None:
            player = cls.deserialize(subs_ref.get())
            return player

        return None
